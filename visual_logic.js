 ...
 * Mon Apr 08 2024 19:22:54 GMT+0530 (India Standard Time)
...
var floor_list, x, j, k, no_collision, piece_list, i, newpiece;
...
function drag_move(x) {
  objConstraintAddLimit('myConst', x, 'POS_Z', 0.90422, 0.90422, '', 0, 'LIMITDIST_ONSURFACE');
  createPhysicsBody('DYNAMIC', x, 'MESH', 1);
  physicsBodyApplyParam('ANGULAR_DAMPING', x, 1);
  physicsBodyApplyParam('LINEAR_DAMPING', x, 1);
  registerOnDrag(x, false, [0], function() {
    setBodyState('MAKE_KINEMATIC', x);
  }, function() {
    dragMove(x, 'XYZ', '!F:*!N^vs(ra+N0fMLxd', '}BeO9_giw_I)rPGR%o%%');
  }, function() {
    setBodyState('MAKE_DYNAMIC', x);
  }, '}BeO9_giw_I)rPGR%o%%');
}
...
if (false) {
    ...
    _pGlob.world.getWorldInfo().set_m_gravity(new Ammo.btVector3(0, -0, 0));
...
_pGlob.world.setGravity( new Ammo.btVector3( 0, -0, 0 ) );
...
registerEveryFrame(function() {
  detectCollision(getObjectsFrom(piece_list, 'ALL'), getObjectsFrom(piece_list, 'ALL'), function() {
    objConstraintAddLimit('myConst', dictGet(_pGlob.collisionInfo, 'objectB'), 'POS_Z', 2, 2, '', 0, 'LIMITDIST_ONSURFACE');
    no_collision = 0;
  }, function() {
    no_collision = 1;
  });
  if (no_collision) {
    registerSetTimeout(2, function() {
      objConstraintAddLimit('myConst', x, 'POS_Z', 0.907, 0.907, '', 0, 'LIMITDIST_ONSURFACE');
    });
  }
});

drop(dictGet(_pGlob.collisionInfo, 'objectB'));
