 ...
 * Wed Apr 10 2024 19:13:37 GMT+0530 (India Standard Time)
...
var m, x, piece_dict, space, j, k, newpiece, i;
...
objConstraintAddLimit('zAxisConstraint', ['ALL_OBJECTS'], 'POS_Z', 0.90422, 100, '', 0, 'LIMITDIST_ONSURFACE');
objConstraintOperations('zAxisConstraint', 'REMOVE', 'Spawn');
objConstraintOperations('zAxisConstraint', 'REMOVE', 'BaseWhite');
objConstraintOperations('zAxisConstraint', 'REMOVE', 'BaseBlack');
space = 1.6;
for (j = 1; j <= 10; j++) {
  k = j % 2;
  if (k == 0) {
    for (i = 1; i <= 10; i += 2) {
      setObjTransform(cloneObject('BaseBlack'), false, 'position', [i * space, j * space, ''], true);
      setObjTransform(cloneObject('BaseWhite'), false, 'position', [(i + 1) * space, j * space, ''], true);
    }
  } else {
    for (i = 1; i <= 10; i += 2) {
      setObjTransform(cloneObject('BaseWhite'), false, 'position', [i * space, j * space, ''], true);
      setObjTransform(cloneObject('BaseBlack'), false, 'position', [(i + 1) * space, j * space, ''], true);
    }
  }
}
